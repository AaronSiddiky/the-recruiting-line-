-- RecruitingLine :: "not interested" is a one-month snooze. Safe to run repeatedly.
--
-- Saving that outcome now sets next_follow_up a month out, and the queue
  ...
  with picked as (
    select c.id, c.name, c.phone
     ...
     where c.do_not_call = false
       ...
       and (c.response is null or c.response::text in ('call_back', 'no_answer', 'not_interested'))
